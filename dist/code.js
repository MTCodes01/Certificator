"use strict";(()=>{var S=(e,t)=>()=>(t||e((t={exports:{}}).exports,t),t.exports);var p=(e,t,a)=>new Promise((n,o)=>{var s=i=>{try{g(a.next(i))}catch(m){o(m)}},c=i=>{try{g(a.throw(i))}catch(m){o(m)}},g=i=>i.done?n(i.value):Promise.resolve(i.value).then(s,c);g((a=a.apply(e,t)).next())});var E=S(w=>{figma.showUI(`<!DOCTYPE html>\r
<html lang="en">\r
\r
<head>\r
...
\r
</html>`,{width:360,height:480,themeColors:!0});var r=null;function R(e){let t=new Set;function a(n){if(n.type==="TEXT"){let s=n.characters.match(/#[a-zA-Z_][a-zA-Z0-9_]*/g);s&&s.forEach(c=>t.add(c.substring(1).toLowerCase()))}"children"in n&&n.children.forEach(o=>a(o))}return a(e),Array.from(t)}function A(e){let t=e.match(/#[a-zA-Z_][a-zA-Z0-9_]*/g);return t?t.map(a=>a.substring(1).toLowerCase()):[]}function u(e,t){return p(this,null,function*(){if(e.type==="TEXT"){let a=e,n=a.characters;yield Promise.all(a.getRangeAllFontNames(0,a.characters.length).map(o=>figma.loadFontAsync(o)));for(let[o,s]of Object.entries(t)){let c=new RegExp(`#${o}`,"gi");n=n.replace(c,s)}a.characters=n}if("children"in e)for(let a of e.children)yield u(a,t)})}function C(e,t){let a=e;for(let[n,o]of Object.entries(t)){let s=new RegExp(`#${n}`,"gi");a=a.replace(s,o)}return a}function v(e){return p(this,null,function*(){if(!r){figma.ui.postMessage({type:"error",error:"No template frame selected"});return}let t=r.width,a=r.height,n=e.length,o=Math.min(20,n),s=Math.ceil(n/o),c=t*.5,g=t-a+20,i=r.x+t+c,m=r.y,f=[];for(let l=0;l<e.length;l++){let d=e[l],M=Math.floor(l/o),N=l%o,x=i+M*(t+c),P=m+N*(a+g),h=r.clone();h.x=x,h.y=P,h.name=C(r.name,d),yield u(h,d),f.push(h),figma.ui.postMessage({type:"progress",current:l+1,total:e.length})}figma.currentPage.selection=f,figma.viewport.scrollAndZoomIntoView(f),figma.ui.postMessage({type:"complete",count:f.length})})}function y(){let e=figma.currentPage.selection;if(e.length===1&&e[0].type==="FRAME"){r=e[0];let t=R(r),a=A(r.name),n=[...new Set([...t,...a])];figma.ui.postMessage({type:"template-selected",name:r.name,placeholders:n})}else r=null,figma.ui.postMessage({type:"no-template"})}figma.on("selectionchange",y);y();figma.ui.onmessage=e=>p(w,null,function*(){if(e.type==="generate"){if(!e.data||e.data.length===0){figma.ui.postMessage({type:"error",error:"No data received"});return}try{yield v(e.data)}catch(t){figma.ui.postMessage({type:"error",error:t instanceof Error?t.message:"Unknown error occurred"})}}e.type==="cancel"&&figma.closePlugin()})});E();})();
